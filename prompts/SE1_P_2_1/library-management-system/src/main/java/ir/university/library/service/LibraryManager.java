...
public class LibraryManager {
    ...
    public boolean requestLoan(String bookId, LocalDate startDate, LocalDate endDate) {
        ...
        // بررسی فعال بودن دانشجو - پرتاب استثنا
        if (!student.isActive()) {
            throw new InvalidStudentStatusException(
                    "حساب کاربری دانشجو غیرفعال است. نام کاربری: " + student.getUsername());
        }
        ...
        // اگر کتاب موجود نبود یا در امانت است - پرتاب استثنا
        if (foundBook == null) {
            throw new BookNotAvailableException(
                    "کتاب مورد نظر موجود نیست یا در حال حاضر امانت داده شده است. شناسه کتاب: " + bookId);
        }
    ...
    public boolean approveLoan(String loanId, String employeeUsername) {
        ...
        // اگر درخواست قبلاً تایید شده باشد - پرتاب استثنا
        if ("APPROVED".equals(foundLoan.getStatus())) {
            throw new InvalidRequestStatusException("درخواست امانت قبلاً تایید شده است. شناسه درخواست: " + loanId);
        }

        if (!"PENDING".equals(foundLoan.getStatus())) {
            System.out.println("❌ درخواست امانت در وضعیت نامناسب برای تایید است!");
            return false;
        }
    ...
    // در LibraryManager اضافه کنید یا اصلاح کنید:
    ...
    // Helper method for testing - get loan by ID
    ...
    // Helper method for testing - create loan directly
    public Loan createLoanDirectly(String loanId, String studentUsername, String bookId,
            LocalDate startDate, LocalDate endDate, String status) {
        Loan loan = new Loan(loanId, studentUsername, bookId, startDate, endDate);
    ...
    public void printStatus() {
        ...
        for (Book book : books) {
            System.out
                    .println("  " + book.getBookId() + ": " + book.getTitle() + " - Available: " + book.isAvailable());
        }
